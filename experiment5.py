...
def experiment(grid_size, stn_pos, tools_pos, l_pos, a_pos, l_tp=None, l_path=[], communication_timesteps=[], debug=False, queryStrat=None):
    ...
    env.register_communication(communication_timesteps, query_type=queryStrat)
...
def get_query_timesteps(grid_size, stn_pos, tools_pos, l_pos, a_pos, l_tp, all_leader_paths, num_query=None, debug=False):
    ...
    queries = [queryOne, queryTwo, queryThree, None]

    query_timesteps = []
    for qp in [0,1,6]:
        for query in queries:
            timesteps = []
            if qp == 0 and query is not None:
                continue
            for path in all_leader_paths:
                steps = experiment(grid_size, stn_pos, tools_pos, l_pos, a_pos, l_tp, list(path), [qp], debug=debug, queryStrat=query)
                #if steps == 16:
                    #input()
...
def create_graphs(grid_size, stn_pos_perm, stn_names, tools_pos, l_pos, a_pos, l_tp_perm, num_query=None):
    ...
    num_graphs = 1
    np.savetxt('figure2_results.dat', query_timesteps)
...
stn_names = ['1', '2', '3']
#stn_pos_perm = [[Point2D(2,7), Point2D(8,7), Point2D(8,3)],
#                [Point2D(2,7), Point2D(8,3), Point2D(8,7)],
#                [Point2D(8,3), Point2D(8,7), Point2D(2,7)]]
stn_pos_perm = [[Point2D(3,8), Point2D(7,8), Point2D(7,2)],
...
l_tp_perm = [agent.AgentType([0]), agent.AgentType([1]), agent.AgentType([2])] # Optional fixed order of stations to pass to agent_leader()
